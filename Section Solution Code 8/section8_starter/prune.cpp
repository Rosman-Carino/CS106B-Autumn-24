...
void removeLeavesV1(TreeNode*& node) {
    // Base Case
    if(node == nullptr) return;

    if (node->left == nullptr && node->right == nullptr) {
        delete node;
        node = nullptr; // Because it is passed by reference we can delete ourself and set it to nullptr
    } else {
        removeLeavesV1(node->left);
        removeLeavesV1(node->right);
    }
}
