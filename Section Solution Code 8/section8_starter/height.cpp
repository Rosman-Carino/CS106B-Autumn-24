...
/**
 * @brief heightV1: Here is 1 Version to Calculate the Height of a Tree
 * @param node
 * @return
 */
int heightV1(TreeNode *node) {
    // Base Case:
    if (node == nullptr) return 0;

    // Recursive Cases
    int leftChild = heightV1(node->left);
    int rightChild = heightV1(node->right);
    int totalHeight = 1 + max(leftChild, rightChild);
    return totalHeight;

}
/**
 * @brief height: Here is an optimal version to calculate the Height of a Tree
 * @param node
 * @return
 */
