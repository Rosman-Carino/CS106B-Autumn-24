...
bool isBalancedV1(TreeNode *node) {
    // ** Base Cases **
    if (node == nullptr) return true; // Success Case

    int leftTreeHeight = treeHeight(node->left);
    int rightTreeHeight = treeHeight(node->right);
    if (abs(leftTreeHeight - rightTreeHeight) > 1) return false; // Failure Case
...
bool isBalanced(TreeNode *node) {
    ...
    int leftTreeHeight = treeHeight(node->left);
    int rightTreeHeight = treeHeight(node->right);
    int heighDifference = abs(leftTreeHeight - rightTreeHeight);
    // ** Recursive Cases **
